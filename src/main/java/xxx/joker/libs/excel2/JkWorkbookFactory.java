...
public class JkWorkbookFactory {
    ...
    public static JkWorkbook create(Path excelPath) {
        if (Files.isDirectory(excelPath)) {
            throw new JkRuntimeException("The input path is a folder (must be and excel2 file). [" + excelPath + "]");
        }

        JkExcelType excelType = JkExcelType.fromExtension(excelPath);
        if (excelType == null) {
            throw new JkRuntimeException("File %s is not an excel2 file", excelPath);
        }
